...
import { type GoalPartDeux, type Repository, type GitHubRepo, type ProjectDataSource, type ProjectPartDeux } from 'lib/data-model-types'
...
export const POST = auth(async (req: NextAuthRequest) => {
    ...
    try {
        ...
        const projectPromises = org.projectIds.map((projectId) => getProject(projectId))
        ...
        if (duplicateProject) {
            return new Response(
                `Project with a name of '${name}' already exists`,
                {
                  status: 400,
                },
              )
        ...
        const goal: GoalPartDeux = {
            ...
            description: 'Sara provides details about your project as she learns them',
