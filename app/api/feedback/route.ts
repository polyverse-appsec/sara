...
import { type Project, type ProjectHealth, type Org, type User } from './../../../lib/data-model-types'
...
export const POST = auth(async (req: NextAuthRequest) => {
    ...
    try {
        ...
        feedbackDescription = reqBody.feedbackDescription ? reqBody.feedbackDescription.trim() : ''
        ...
        if (feedbackTitle.length === 0 && feedbackDescription.length === 0) {
            logger.infoWithContext('User provided feedback without any content', { user })
        ...
        body += `${feedbackDescription && feedbackDescription.length > 0 ? 'Feedback: ' + feedbackDescription : 'No description provided (check title)'}`
        ...
        await octokit.request(`POST /repos/polyverse-appsec/sara/issues`, {
            ...
            repo: 'sara',
            title: feedbackTitle && feedbackTitle.length > 0 ? feedbackTitle : 'No title provided (check for details)',
            body,
        ...
        logger.errorWithContext(`Failed to leave user feedback${feedbackTitle && feedbackTitle.length > 0 ? ' ' + feedbackTitle : ''}${feedbackDescription && feedbackDescription.length > 0 ? ' ' + feedbackDescription : ''}`, {user, error} as SaraLogContext)
