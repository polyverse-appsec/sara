...
import { type ProjectDataReference, type User } from '../../lib/data-model-types'

const setCachedProjectUserFileInfos = async (projectName: string, user: User, fileInfos: ProjectDataReference[]): Promise<void> => {
    ...
    const setFileInfoPromises = fileInfos.map((fileInfo) => setProjectUserFileInfo(projectName, user.id, fileInfo))
    ...
    const setFileInfoIdsPromises = fileInfos.map((fileInfo) => setProjectUserFileInfoId(projectName, user.id, fileInfo.id))
