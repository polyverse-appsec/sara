...
import { OrgPartDeux, type GitHubRepo } from './../../../../../lib/data-model-types'
...
const PrimaryDataSourceSelector = ({
  ...
  const [githubReposForOrgs, setGitHubReposForOrgs] = useState<Record<string, GitHubRepo[]>>({})
  const [githubAppInstalledForOrgs, setGitHubAppInstalledForOrgs] = useState<Record<string, string>>({})
  ...
  const [shouldShowLoadingSpinner, setShouldShowLoadingSpinner] =
    ...
    async function fetchAndSetReposForOrgs(orgs: OrgPartDeux[]) {
        ...
        async function setAppInstalledStatusForOrgs(orgs: OrgPartDeux[]) {
            if (!saraSession) {
    ...
    <div className="p-4 space-y-1 text-sm border rounded-md">
      <DropdownMenu>
        ...
        <DropdownMenuContent sideOffset={8} align="start" className="min-w-64 max-h-60">
          {orgs.map((org: OrgPartDeux) => (
            <Collapsible 
                key={org.name}
                className={githubAppInstalledForOrgs[org.name] === 'INSTALLED' ? "" : "select-none opacity-50 grayscale pointer-events-none"}
            >
              <CollapsibleTrigger>
                <div className="flex items-center">
                  <DropdownMenuLabel>{org.name}</DropdownMenuLabel>
                  {githubAppInstalledForOrgs[org.name] !== 'INSTALLED' ?
                    <div title="Please install github app for this org to access its repos" className="pointer-events-auto">
