...
const addFineTuningTagToChatQuery = async (fineTuningTag: FineTuningTags, chatQueryId: string, email: string): Promise<void> => {
    ...
    await kv.sadd(globalSetKey, chatQueryId)

    if (userSetKey) {
        await kv.sadd(userSetKey, chatQueryId)
    }
}

export default addFineTuningTagToChatQuery
