...
import { globalPromptFileInfoIdsSetKey, promptFileInfoKey, relatedPromptFileInfosToProjectIdsSetKey } from './keys'

const deletePromptFileInfo = async (promptFileInfoId: string, parentProjectId: string): Promise<void> => {
    const itemKey = promptFileInfoKey(promptFileInfoId)

    // Remove the tracked prompt file info its project relationship set of
    // IDs...
    const promptFileInfosToProjectIdsSetKey = relatedPromptFileInfosToProjectIdsSetKey(parentProjectId)
