...
const deleteTask = async (
  ...
  parentId: ParentIdentifiers,
    ...
    if (parentId.parentGoalId && parentId.parentTaskId) {
        throw new Error(`Both parent goal ID and parent task ID set when deleting task '${taskId}'`)
    }
    ...
    if (parentId.parentTaskId) {
        ...
        // parent goal.
        const childTaskIds = (await kv.zrange(childTaskToParentTaskIdsSetKey, 0, -1)) as string[]
