...
import { globalTaskIdsSetKey, relatedChildTasksToParentGoalIdsSetKey, relatedChildTasksToParentTaskIdsSetKey, taskKey } from './keys'
...
const createTask = async (task: TaskPartDeux): Promise<void> => {
    ...
    if (task.parentGoalId === null && task.parentTaskId === null) {
        throw new Error(`Failed to write task to data store as neither the 'parentGoalId' or the 'parentTaskId' is set`)
    }

    if (task.parentGoalId !== null && task.parentTaskId !== null) {
        throw new Error(`Failed to write task to data store as both the 'parentGoalId' and the 'parentTaskId' are set`)
    }
    ...
    if (task.parentGoalId !== null) {
        const relatedTasksToParentGoalIdsSetKey = relatedChildTasksToParentGoalIdsSetKey(task.parentGoalId)
    ...
    if (task.parentTaskId !== null) {
        const relatedTasksToParentTaskIdsSetKey = relatedChildTasksToParentTaskIdsSetKey(task.parentTaskId)
