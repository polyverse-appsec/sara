...
import { globalProjectIdsSetKey, projectKey, relatedPromptFileInfosToProjectIdsSetKey } from './keys'

const deleteProject = async (projectId: string): Promise<void> => {
    // Since there is a relationship set of IDs for prompt file infos start by
    // deleting those...
    const promptFileInfosToProjectIdsSetKey = relatedPromptFileInfosToProjectIdsSetKey(projectId)
    const promptFileInfoIds = (await kv.zrange(promptFileInfosToProjectIdsSetKey, 0, -1)) as string[]

    if (promptFileInfoIds.length > 0) {
        const deletePipeline = kv.pipeline()
        promptFileInfoIds.forEach((promptFileInfoId) => deletePipeline.del(promptFileInfoId))
