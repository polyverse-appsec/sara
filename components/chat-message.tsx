...
function renderAvatarAndLoadingSpinner(message: any, renderLoadingSpinner: boolean) {
  return (
    <div className={"flex flex-col items-center"}>
      <div
        className={cn('flex h-8 w-8 shrink-0 select-none items-center justify-center rounded-md border shadow',
          message.role === 'user'
          ? 'bg-background'
...
export function ChatMessage({ message, renderLoadingSpinner, ...props }: ChatMessageProps) {
  return (
    <div
      className={cn('group relative mb-4 flex items-start md:-ml-12')}
      {...props}
    >
      {renderAvatarAndLoadingSpinner(message, renderLoadingSpinner)}
